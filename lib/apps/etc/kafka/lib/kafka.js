...
const logger = runtime_1.Logger.getLogger('app:kafka');
const versions = {
    '3.7': '3.7.0'
};
class KafkaController extends runtime_1.ClusterAppController {
    deploy() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const specs = [];
            const svcs = yield this.get('route', request.name);
            const svcmap = svcs &&
                svcs.reduce((o, svc) => {
                    if (!svc || !svc.name || !(svc.name === request.name || svc.name.startsWith(`${request.name}-`)))
                        return o;
            ...
            if (replicas <= 0 || replicas % 2 === 0)
                throw new Error('replicas must be odd');
            const voters = ','
                .repeat(replicas - 1)
                .split(',')
